...
| `ProblemDetailsHttpExtensions` | `HttpResponseMessage` members: `ReadProblemAsync` and `ReadOneOfAsync<T>`. A failed response always yields a problem, synthesised from the status code when the body has none. Expected failures are returned, never thrown; only a malformed success body throws, as a `JsonException`. |
