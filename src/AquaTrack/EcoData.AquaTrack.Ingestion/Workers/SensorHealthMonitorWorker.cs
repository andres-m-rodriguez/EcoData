...
) : BackgroundService
{
    ...
    private async Task CheckSensorHealthAsync(CancellationToken cancellationToken)
    {
        ...
        {
            var config = status.Sensor?.HealthConfig;
            if (config is null)
            {
                continue;
            }
            ...
            var newStatus = DetermineHealthStatus(status, config, now);
            ...
            switch (newStatus)
            {
                case SensorHealthStatusType.Stale when previousStatus == SensorHealthStatusType.Healthy:
                    ...
                    await healthRepository.CreateAlertAsync(
                        ...
                        cancellationToken);
                    logger.LogWarning("Sensor {SensorId} ({SensorName}) is now stale. Last reading: {LastReadingAt}",
                        status.SensorId, status.Sensor?.Name, status.LastReadingAt);
                    break;
                ...
                case SensorHealthStatusType.Unhealthy when previousStatus != SensorHealthStatusType.Unhealthy:
                    ...
                    await healthRepository.CreateAlertAsync(
                        ...
                        cancellationToken);
                    logger.LogError("Sensor {SensorId} ({SensorName}) is now unhealthy. Last reading: {LastReadingAt}",
                        status.SensorId, status.Sensor?.Name, status.LastReadingAt);
                    break;
                ...
                case SensorHealthStatusType.Healthy when previousStatus != SensorHealthStatusType.Healthy:
                    ...
                    await healthRepository.CreateAlertAsync(
                        ...
                        cancellationToken);
                    logger.LogInformation("Sensor {SensorId} ({SensorName}) has recovered",
                        status.SensorId, status.Sensor?.Name);
                    break;
            }
        }
        ...
        DateTimeOffset now)
    {
        ...
        var staleThreshold = TimeSpan.FromSeconds(config.StaleThresholdSeconds);
        var unhealthyThreshold = TimeSpan.FromSeconds(config.UnhealthyThresholdSeconds);
