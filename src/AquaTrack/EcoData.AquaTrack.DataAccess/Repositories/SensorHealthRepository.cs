...
public sealed class SensorHealthRepository(IDbContextFactory<AquaTrackDbContext> contextFactory)
    : ISensorHealthRepository
{
    ...
    public async Task<IReadOnlyList<SensorHealthStatus>> GetMonitoredStatusesWithConfigAsync(
        CancellationToken cancellationToken = default)
    {
        await using var context = await contextFactory.CreateDbContextAsync(cancellationToken);
