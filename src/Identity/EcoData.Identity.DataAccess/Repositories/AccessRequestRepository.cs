    ...
    : IAccessRequestRepository
{
    ...
    public async Task<AccessRequest?> GetByEmailAsync(string email, CancellationToken cancellationToken = default)
    {
        await using var context = await contextFactory.CreateDbContextAsync(cancellationToken);
        return await context.AccessRequests
            .FirstOrDefaultAsync(ar => ar.Email.ToLower() == email.ToLower(), cancellationToken);
    }

    public async Task<AccessRequest> CreateAsync(AccessRequest accessRequest, CancellationToken cancellationToken = default)
    {
        await using var context = await contextFactory.CreateDbContextAsync(cancellationToken);
    ...
    public async Task<AccessRequest> UpdateAsync(AccessRequest accessRequest, CancellationToken cancellationToken = default)
    {
        await using var context = await contextFactory.CreateDbContextAsync(cancellationToken);
        ...
        [EnumeratorCancellation] CancellationToken cancellationToken = default)
    {
        ...
        if (!string.IsNullOrWhiteSpace(parameters.Search))
        {
            ...
            query = query.Where(ar =>
                ar.Email.ToLower().Contains(search)
                || ar.DisplayName.ToLower().Contains(search)
            );
        }
